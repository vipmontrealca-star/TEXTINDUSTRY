...
- [ ] **M6c** — Subdomain setup (in progress, 2026-07-03). **Bonus simplification discovered:** user had separately switched GoDaddy nameservers to HostGator's (`hgns1.hostgator.com`/`hgns2.hostgator.com`) — confirmed via `nslookup` — so HostGator's cPanel Zone Editor is now authoritative for all DNS on this domain, meaning creating the two subdomains in cPanel auto-provisions their DNS records with no manual GoDaddy step needed.
  ...
  - [ ] **Current step, waiting on user:** create the two dedicated FTP accounts (`deploy_fr` scoped to `fr.textindustry.com`, `deploy_ar` scoped to `ar.textindustry.com`), each verified for stray folders immediately after creation per [[feedback_cpanel_ftp_deploy_verification]] — instructions given, not yet confirmed done (got sidetracked by the About Us page / photo request, M5a/M5c above).
  - [ ] Remaining after that: a build step to generate per-language static HTML from `translations.js`, a 3-target deploy workflow, `hreflang` tags across all three variants, and sitemap updates.
