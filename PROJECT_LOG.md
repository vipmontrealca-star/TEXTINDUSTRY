...
- [ ] **M6c** — Subdomain setup (in progress, 2026-07-03). **Bonus simplification discovered:** user had separately switched GoDaddy nameservers to HostGator's (`hgns1.hostgator.com`/`hgns2.hostgator.com`) — confirmed via `nslookup` — so HostGator's cPanel Zone Editor is now authoritative for all DNS on this domain, meaning creating the two subdomains in cPanel will auto-provision their DNS records with no manual GoDaddy step needed (simpler than the M3.4-era GoDaddy-A-record approach). **Current step, waiting on user:** create `fr.textindustry.com` and `ar.textindustry.com` in cPanel → Domains, with document root explicitly typed as `fr.textindustry.com` / `ar.textindustry.com` (siblings of the existing `textindustry.com` folder, not nested inside it) — then verify the actual resulting path in File Manager *before* creating FTP accounts for them, learning from M3.6. Remaining steps after that: dedicated FTP accounts (2x, each path-verified immediately after creation), a build step to generate per-language static HTML from `translations.js`, a 3-target deploy workflow, `hreflang` tags across all three variants, and sitemap updates.
