...
### 2026-07-03 — Follow-up: mail.textindustry.com now redirects to the gateway
**Client ask:** wanted `mail.textindustry.com` itself to lead to the branded gateway page, not just have `webmail.html` exist as a separate bookmark.

**Delivered:** `.htaccess` (main domain document root) brought under version control for the first time — it previously existed only on the server, generated by cPanel's Force-HTTPS-Redirect feature, never in the repo. Got its exact existing contents from the client via File Manager before touching anything, to avoid blindly overwriting server config we didn't have a copy of. Added one new rule after the existing HTTPS/canonicalization block: any request to `mail.textindustry.com` (any path) now 302-redirects to `https://textindustry.com/webmail.html`.

**Risk called out explicitly before deploying:** a syntax error in `.htaccess` can 500 the *entire* main site, and Apache rewrite behavior can't be tested in this environment's dev preview (it's a plain static file server, not Apache) — so this shipped without being test-run first, only carefully written and reviewed by eye.

**Verified immediately after deploy, both the new behavior and a regression check on everything the existing rule/other domains do:**
- `mail.textindustry.com` → `302` → `Location: https://textindustry.com/webmail.html`, and following it loads (200).
- `textindustry.com` still `200`, `www.textindustry.com` still `301` → `https://textindustry.com/` (the pre-existing rule, untouched and still working).
