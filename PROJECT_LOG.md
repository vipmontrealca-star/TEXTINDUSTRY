...
- [ ] **M6c** — Subdomain setup (in progress, 2026-07-03). **Bonus simplification discovered:** user had separately switched GoDaddy nameservers to HostGator's (`hgns1.hostgator.com`/`hgns2.hostgator.com`) — confirmed via `nslookup` — so HostGator's cPanel Zone Editor is now authoritative for all DNS on this domain, meaning creating the two subdomains in cPanel auto-provisions their DNS records with no manual GoDaddy step needed.
  ...
  - [ ] **Still needed before this can go live:** 4 new GitHub repo secrets (`HOSTGATOR_FTP_FR_USERNAME`, `HOSTGATOR_FTP_FR_PASSWORD`, `HOSTGATOR_FTP_AR_USERNAME`, `HOSTGATOR_FTP_AR_PASSWORD`) — reuses the existing `HOSTGATOR_FTP_SERVER` secret since all FTP accounts share one server. Not yet committed/pushed either — waiting to gather the secrets first so the very first workflow run against this new config has a real chance of succeeding.
